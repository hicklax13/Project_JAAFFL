...
rules** (provided verbatim). The current map is a CBS-Standard _guess_; several values are wrong and
...
values. The scoring _evaluator_ (`league.scoring.league_points`), the roster/starter model, and the
...
JAAFFL2025 map, authoritative — only CBS live-frame _parsing_ stays capture-blocked, not the scoring
